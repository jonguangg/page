...
//step 4 将让传后的文件名改名 
$fileimgweb= "./backup/";    
$randname  = "channelList_".date("Y").date("m").date("d")."_".date("H").date("i").date("s")."_".rand(100, 999).".".$kuoZhanMing;
setCookie("randname", $randname, time()+3600);//	记录上传的excel文件名,给excelImpor.php读取
//将临时位置的文件移动到指定的目录上即可      
if(is_uploaded_file($_FILES["excel"]["tmp_name"])){
	if(move_uploaded_file($_FILES["excel"]["tmp_name"],$fileimgweb.$randname)){
	//	echo "<script type='text/javascript'>alert('上传成功');</script>";
		echo '<div style="position:absolute;top:0px;left:0px;width:100%;height:100%;background:url(img/loading.gif); background-size:100% 100%;"></div>';		
