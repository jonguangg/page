	...
	function registedVipCard(){
		...
		getID("title").innerHTML = (language=="c")?"<span style=font-size:80px;''>延長授權日期</span>":"Registered VIP Card";
		getID("cardKeyTitle").innerHTML = (language=="c")?"請輸入卡密":"Please enter license code";
		getID("back").innerHTML = (language=="c")?"<span style='font-size:60px;'>返&emsp;回</span>":"Back";
		getID("ok").innerHTML = (language=="c")?"<span style='font-size:60px;'>提&emsp;交</span>":"Submit";
		scrollDisable();
		indexArea = "register";
	}
	...
	function checkInput(){	//提交授权码
		...
		/*if( getID("card_id").value.length<8 ){
			...
			getID("msg").style.background = "linear-gradient(to bottom,gray,white)";
			getID("msg").innerHTML = "License code error !";
		}else{
