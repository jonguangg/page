	...
	function showZhiBoList(_pageNum){
		...
		for(i=0; i<10; i++){			
			...
			getID("zhiBo").innerHTML += '<div id=zhiBoImg'+(i+pageZhiBo*10)+' class="zhiBoImg" style="height:'+clientHeight+'px; background:url('+poster+')" onclick=playZhiBo("'+zhiBoArr[(i+pageZhiBo*10)].url+'") ><div class="zhiBoName">'+decodeURIComponent(zhiBoArr[(i+pageZhiBo*10)].title)+'</div></div>';			
