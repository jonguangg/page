<?php
	...
	*/
    ...
    //  查询节目ADI信息
    $sql1 = mysqli_query($connect,"SELECT * from video WHERE id='$id' LIMIT 0,1 ") or die(mysqli_error($connect));

    while($row=mysqli_fetch_array($sql1)){      //遍历查询结果，将每条结果写入数组    
        $detailJson['id'] = $row['id'];          //用于进入详情页播放上次播放的那集
        $father =  $row['father'];
    ...
    while($row=mysqli_fetch_array($sql2)){ //遍历查询结果，将集数信息写入数组  
        $detailJson['list'][] = array(
            'id' => $row['id'],             //用于选集时点击播放
		//	'name' => $row['name'],
			'episode' => $row['episode'],   //用于显示选集时的集数
		);  
    }
    ...
    if( mysqli_num_rows($sql3) > 0 ){
        while($row=mysqli_fetch_array($sql3)){ //遍历查询结果，将结果写入数组    
            $detailJson['id'] = $row['id'];                 //用于进入详情页播放上次播放的那集
            $detailJson['episodePos'] = $row['episode'];    //用于定位选集第几集
        } 
    }else{
    ...
    //  查询收藏记录
    $sql3 = mysqli_query($connect,"SELECT * from collect WHERE sn='$sn' AND id='$id' ORDER BY editTime ASC ") or die(mysqli_error($connect));
    if( mysqli_num_rows($sql3)>0 ){
    ...
    for($i=1; $i<sizeof($tagArr)-1; $i++){
        $sql4 = mysqli_query($connect,"SELECT * from video WHERE statuss=1 AND id!='$id' AND tag LIKE '%$tagArr[$i]%' ORDER BY playNum DESC limit 0,1 ") or die(mysqli_error($connect));
    ...
    $sql5 = mysqli_query($connect,"SELECT * from video WHERE statuss=1 AND id!='$id' ORDER BY uploadTime DESC limit 0,20 ") or die(mysqli_error($connect));
