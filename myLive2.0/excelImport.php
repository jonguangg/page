...
if (stripos($fileName, "hannelList")) {
	...
	for ($i = 1; $i <= $row; $i++) {
		...
		if ($i > 1) { //如果没有表头就用0
			...
			$sql = mysqli_query($connect, "replace into channel(groupId,groupName,channelName,videoUrl,channelLogo) values ($excelData[0],'$excelData[1]','$excelData[4]','$excelData[5]','$excelData[6]')") or die(mysqli_error());
		}
	}
