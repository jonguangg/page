            ...
            $.each(list,
                function(index, array) { //遍历json数据列
                    ...
                    if( typeof(window.androidJs) != "undefined"){
                        getID("shcContent").innerHTML += '<div id=videoImg'+id+' class="vodListImg" style="height:'+imgHeight+';background:url(../vod/'+name+'/'+poster+')" ><div style="float:left;left:0px;top:0px;width:80%;height:100%;" onClick=window.androidJs.JsPlayVod("'+playUrl+'");></div><img id=collectImgs'+id+' src='+collectImg+' style="float:left;margin-left:20px;margin-top:20px;width:100px;height:100px;" onClick=changeCollect('+id+') /></div><div class="vodListName">'+father2+'</div>';
                    }else{
                        getID("shcContent").innerHTML += '<div id=videoImg'+id+' class="vodListImg" style="height:'+imgHeight+';" ><video id=h5Video'+id+' style="object-fit:fill;" width="100%" height="100%" controls preload="auto" type="application/x-mpegURL" poster="../vod/'+name+'/'+poster+'" src='+playUrl+' playsinline x5-playsinline webkit-playsinline x-webkit-airplay="true" x5-video-player-fullscreen="true" x5-video-orientation="landscape"></video></div><div class="vodListNameBrowser">'+father2+'</div><img id=collectImgs'+id+' src='+collectImg+' style="float:left;width:100px;height:100px;" onClick=changeCollect('+id+') />';
                    }
                });
            setTimeout(function() {
                changePageStatus = "t";
